...
/* SPI registers Masks */
#define CR1_CLEAR_Mask       ((u16)0x0040) // was 0x3040 juan clean up
...
{
  volatile u16 wDummy; /* create for read of the SPI->DR, required to work
			around an apparent bug in silicon */
	
	/* Check the parameters */
  assert_param(IS_SPI_ALL_PERIPH(SPIx));

	/* Work-around for SPI problem: on a single-byte SPI slave- read operation,
			unless a "dummy" read is performed on the SPI DR (data register) before
			the '0' is written to DR, the data that is subsequently clocked	into the
			STM SPI port (i.e. the response from the slave device) will not
			be transferred to the DR. When the DR is later read to retrieve the data
			that should have been received from the slave device, the data in the DR
			will be garbage (i.e. not the data that the slave device drove onto	the
			MISO line).
				...
				...example source code
			       ...
			       SPI_I2S_SendData(SPI1, request);
				     while (SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET);
							...
							/////// work-around required here
							/////// SPI1-DR must have a read-access executed
							/////// before the '0' is written to SPI1-DR
							///////	or the data on MISO will not make it into
							/////// SPI1-DR
						 SPI_I2S_SendData(SPI1, 0);

			...more

			The cause of the problem was discovered while single-stepping using the
			JTAG debugger. It was observed that as long as the SPI1->DR register was
			being monitored in a watch-window, the data observed on the SPI bus
			MISO line (via an oscilloscope) would be transferred to the SPI1->DR
			register. If this watch window did not have SPI1->DR in context at the
			time that the '0' was written to the SPI1->DR, the data on the SPI MISO
			would not make it into the DR. It appears that by having SPI1->DR in
			context, the DR will have a read-access occur at every step, including the
			crucial step, the step preceeding the '0' write.
	*/
	wDummy = SPIx->DR; /* perform work-around read */
